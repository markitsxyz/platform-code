...
/**
 * Check if a wallet owns a markits Platform Pass NFT
 */
export async function checkNFTPass(
  ...
  try {
    ...
    // Check each NFT to see if it's a markits Platform Pass
    for (const nftAccount of nftAccounts) {
      ...
      try {
        ...
        if (metadata) {
          // Parse metadata to check if it's a markits Platform Pass
          const metadataDecoded = parseMetadata(metadata.data);
          
          if (ismarkitsPlatformPass(metadataDecoded)) {
            console.log('Found markits Platform Pass!');
            return {
              hasPass: true,
              nftMint: mintAddress,
    ...
    console.log('No markits Platform Pass found');
...
/**
 * Check if the metadata indicates this is a markits Platform Pass
 */
function ismarkitsPlatformPass(metadata: any): boolean {
  ...
  // Check for markits Platform Pass indicators
  return name.includes('sol studio platform pass') || 
         name.includes('markits platform pass') ||
         name.includes('markits pass') ||
         symbol.includes('markits Pass') ||
         symbol.includes('markits') ||
